...
- A `-` may also indicate a *sukūn* or may be used to separate letters which when together may represent another letter.
- The letter used for each vowel will always remain consistent to prevent confusion.
- A `t` or `h` followed by a `,` indicates a *tā marbūṭah,*.
